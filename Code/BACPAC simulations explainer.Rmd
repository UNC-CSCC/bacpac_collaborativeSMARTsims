...
```{r packages and scripts, echo = FALSE, warning = FALSE, message = FALSE}
library(tidyverse)
library(DynTxRegime)
source("BACPAC simulations.R")
```
...
a. Generate participants: `makePpts()`
b. Generate covariates: `makeCovariates()`

Step 2: Generate the trial

a. Allocate first-line treatments: `allocateStage1Treatments()`
b. Generate Y1 and responder status: `generateY1()`, `assignResponderStatus()`
c. Allocate second-line treatments: `allocateStage2Treatments()`
d. Generate Y2 `generateY2()`
e. Generate Y `generateY()`
...
Because the design of the trial is evolving, the simulation code is written with maximum flexibility and tractability in mind. Every simulation study starts with a plan which includes a design and a mechanism for generating participants and their outcomes. **The design and participants are operationalized through 8 functions where each function represents a key design or participant generation aspect.** The functions are `makePpts()`, `makeCovariates()`, `allocateStage1Treatments()`, `generateY1()`, `assignResponderStatus()`, `allocateStage2Treatments()`, `generateY2()`, `generateY()`. **The fundamental code design principle is to be tidy.** Specifically, the fundamental object is a dataframe that is piped through the 8 key functions and a dataframe is the yielded output. Once more of the design is nailed down, we may want to re-code this to utilize faster data objects.

The first function is `makePpts()`. It is parameterized by an integer `N` and returns a dataframe with `N` rows and a single variable `ptid` (patient identifier).
