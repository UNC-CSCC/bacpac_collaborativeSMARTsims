...
stage1_param_df <- read.csv("./ParameterCSVs/ThreeTrts-SgSc2-ATESc1/sg2_stage1.csv")

stage1_param_vec <- stage1_param_df$Coefficient
names(stage1_param_vec) <- stage1_param_df$Parameter

stage2_param_df <- read.csv("./ParameterCSVs/ThreeTrts-SgSc2-ATESc1/sg2_stage2.csv")
...
stage1_args <- list( stage.suffix = 1,
                     ...
                     arm.var.name = "A1",
                     true.model.formula = formula(~ -1 + X_3 + A1_0 + A1_1 + A1_2 + A1_1:X_1 + A1_2:X_2 + A1_0:W_1), 
...
stage2_args <- list(stage.suffix = 2,
                    ...
                    arm.var.name = "A2",
                    true.model.formula = formula(~ -1 + X_3 + A2_0 + A2_1 + A2_2 + A2_1:X_1 + A2_0:A2_1 + A2_0:A2_2 + A2_1:A2_2 + A2_2:X_2 + A2_0:W_1), 
...
analysis_args <- list(AnalysisModeling_fn = "QLearning",
                      AnalysisModeling_args = list(stage1.formula = formula(PseudoY ~ -1 + X_3 + A1_0 + A1_1 + A1_2 + A1_1:X_1 + A1_2:X_2 + A1_0:W_1),
                                                   stage2.formula = formula(Y ~ -1 + X_3 + I(A1_0 + A2_0) + I(A1_1 + A2_1) + I(A1_2 + A2_2) + 
                                                                              I(A1_1 + A2_1):X_1 + I(A1_2 + A2_2):X_2  + I(A1_0 + A2_0):W_1),
...
if (gen_oos_flag == TRUE) {
  ...
  if (FALSE) {
    ...
    ptids_to_check<- oosData%>% distinct_at(.vars = c("X_1", "X_2", "X_3"), .keep_all = TRUE )%>% pull(ptid)
    
    oosData %>% filter(ptid %in% ptids_to_check, Mu == maxMu) %>% select(A1, A2, X_1, X_2, X_3, Mu) %>% arrange(X_1, X_2, X_3)
    
    oosData %>% filter(ptid %in% ptids_to_check) %>% 
      select(A1, A2, X_1, X_2, X_3, Mu, maxMu) %>%  
      mutate(Delta = maxMu - Mu) %>% 
      arrange(X_1, X_2, Mu) %>% 
      filter(X_3 == -1) %>% 
      as.data.frame
  }
